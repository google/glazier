...
class AutobuildTest(absltest.TestCase):
  ...
  def test_setup_task_list_success(self, mock_check_winpe):
    # Host
  ...
  def test_setup_task_list_error(self, mock_os, mock_log_and_exit):
    ...
    mock_log_and_exit.assert_called_with(
        'Unable to remove task list', self.autobuild._build_info, 4303,
        mock.ANY)
  ...
  def test_run_build_success(
    ...
    self.autobuild.RunBuild()
    mock_log_and_exit.assert_called_with(
        'Failed to build the task list', self.autobuild._build_info, 4302,
        mock.ANY)
    ...
    self.autobuild.RunBuild()
    mock_log_and_exit.assert_called_with(
        'Failed to execute the task list', self.autobuild._build_info, 4304,
        mock.ANY)

  @mock.patch.object(title, 'set_title', autospec=True)
  def test_run_build_keyboard_interrupt(self, mock_set_title):
  ...
  @mock.patch.object(autobuild.terminator, 'log_and_exit', autospec=True)
  @mock.patch.object(title, 'set_title', autospec=True)
  def test_run_build_glazier_error(self, mock_set_title, mock_log_and_exit):
  ...
  @mock.patch.object(autobuild.terminator, 'log_and_exit', autospec=True)
  @mock.patch.object(title, 'set_title', autospec=True)
  def test_run_build_exception(self, mock_set_title, mock_log_and_exit):
