...
from glazier.lib import splice as public_splice
...
class SpliceDomainJoinTest(test_utils.GlazierTestCase):
  ...
  @mock.patch.object(public_splice.Splice, 'domain_join', autospec=True)
  def test_default(self, mock_domain_join, mock_buildinfo):
    self._splice = splice.SpliceDomainJoin([], mock_buildinfo)
    self._splice.Run()
    mock_domain_join.assert_called_with(mock.ANY, 5, True, True, '')
  ...
  def test_custom(self, mock_domain_join, mock_buildinfo):
    self._splice = splice.SpliceDomainJoin([1, False, False, 'baz'],
                                           mock_buildinfo)
    self._splice.Run()
    mock_domain_join.assert_called_with(mock.ANY, 1, False, False, 'baz')

  @mock.patch('glazier.lib.buildinfo.BuildInfo', autospec=True)
  @mock.patch.object(public_splice.Splice, 'domain_join', autospec=True)
  def test_default_error(self, mock_domain_join, mock_buildinfo):
    self._splice = splice.SpliceDomainJoin([], mock_buildinfo)
    mock_domain_join.side_effect = public_splice.DomainJoinError(5)
    with self.assert_raises_with_validation(splice.ActionError):
      self._splice.Run()
