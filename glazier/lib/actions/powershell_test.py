...
class PowershellTest(parameterized.TestCase):
  ...
  def test_ps_script(self, mock_cachefromline, mock_runlocal):
    ...
    mock_runlocal.side_effect = powershell.powershell.Error
  ...
  def test_ps_script_reboot_no_retry(self, mock_cachefromline, mock_runlocal):
    ...
    mock_runlocal.assert_called_with(
        mock.ANY, SCRIPT_PATH, ARGS, [0, 1337, 1338])
  ...
  def test_ps_script_log(self, mock_cachefromline, mock_powershell):
    ...
    ps = powershell.PSScript(
        [SCRIPT, ARGS, [0], [], False, True, False], self.bi)
    mock_powershell.return_value.RunLocal.return_value = 0
    ps.Run()
  ...
  def test_ps_script_validate_type(
      self, script, ps_args, success_codes, reboot_codes, retry_on_restart,
      shell, log):
    ps = powershell.PSScript(
        [
  ...
  @mock.patch.object(
      powershell.powershell.PowerShell, 'RunCommand', autospec=True)
  def test_ps_command_error(self, mock_runcommand):
    ps = powershell.PSCommand([COMMAND, [1337]], None)
    mock_runcommand.side_effect = powershell.powershell.Error
    with self.assertRaises(powershell.ActionError):
      ps.Run()
  ...
  def test_ps_command_reboot_no_retry(self, mock_runcommand):
    ...
    self.assertEqual(cm.exception.retry_on_restart, False)
    mock_runcommand.assert_called_with(
        mock.ANY, TOKENIZED_COMMAND, [0, 1337, 1338])
  ...
  def test_ps_command_validate_type(
      self, command, success_codes, reboot_codes, retry_on_restart, shell, log):
