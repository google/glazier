...
class CacheTest(absltest.TestCase):
  ...
  def mock_transform(self, string, unused_info):
    if '#' in string:
  ...
  def test_cache_from_line(self, mock_download_file, mock_transform):
    ...
    mock_transform.side_effect = self.mock_transform
    ...
    mock_download_file.assert_has_calls([call1, call2])
    # download exception
    transfer_err = cache.download.DownloadError('Error message.')
    mock_download_file.side_effect = transfer_err
