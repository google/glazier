...
# def _raise_from(outer_exception, inner_exception):
#   try:

#     # Just raise the inner exception.
#     try:
#       raise inner_exception

#     # Catch it and wrap it in the outer exception.
#     except Exception as e:  # pylint: disable=broad-except
#       raise outer_exception from e

#   # Now catch the outer exception and return it.
#   except Exception as result_exception:  # pylint: disable=broad-except
#     return result_exception


def _raise_from(*exception_chain):

  # result_exception = args[0]
  # exception_chain = *args
  ...
  for current_ex in exception_chain[1:]:
    ...
    except Exception as new_previous_ex:
...
class GlazierErrorTest(parameterized.TestCase):
  ...
  def testStr(self, error_code, message, exception_chain, expected_str):
