...
var (
	...
	comRelease                   = defaultCOMRelease
	comSetGUID                   = defaultCOMSetGUID
	comGetAllocatedString        = defaultCOMGetAllocatedString
	comGetUINT32                 = defaultCOMGetUINT32
	comGetGUID                   = defaultCOMGetGUID
	...
	setupDiDestroyDeviceInfoList = func(d windows.DevInfo) error {
		return d.Close()
	}
...
func defaultCoInitializeEx(pvReserved uintptr, dwCoInit uint32) error {
	r1, _, _ := syscall.SyscallN(
		procCoInitializeEx.Addr(), pvReserved, uintptr(dwCoInit),
	)
	if int32(r1) < 0 {
...
func defaultMFStartup(version uint32, flags uint32) error {
	r1, _, _ := syscall.SyscallN(
		procMFStartup.Addr(), uintptr(version), uintptr(flags),
	)
	if int32(r1) < 0 {
...
func defaultMFCreateAttributes(ppMFAttributes **comObject, cInitialSize uint32) error {
	r1, _, _ := syscall.SyscallN(
		procMFCreateAttributes.Addr(),
		uintptr(unsafe.Pointer(ppMFAttributes)),
...
func defaultCoTaskMemFree(pv unsafe.Pointer) {
	if pv != nil {
		syscall.SyscallN(procCoTaskMemFree.Addr(), uintptr(pv))
	}
}

// COM Helpers

func defaultCOMRelease(c *comObject) {
	if c == nil || c.vtbl == nil {
		return
	}
...
func defaultCOMSetGUID(c *comObject, key, val *windows.GUID) error {
	if c == nil || c.vtbl == nil {
...
func defaultCOMGetAllocatedString(c *comObject, key *windows.GUID) (string, error) {
	if c == nil || c.vtbl == nil {
		return "", errors.New("nil COM interface pointer")
	}
	var strPtr *uint16
	...
	if int32(r1) < 0 || strPtr == nil {
		return "", fmt.Errorf(
			"IMFAttributes::GetAllocatedString failed: 0x%08X", uint32(r1),
		)
	}
...
func defaultCOMGetUINT32(c *comObject, key *windows.GUID) (uint32, error) {
	if c == nil || c.vtbl == nil {
...
func defaultCOMGetGUID(c *comObject, key *windows.GUID) (windows.GUID, error) {
	if c == nil || c.vtbl == nil {
	...
	if int32(r1) < 0 {
		return windows.GUID{}, fmt.Errorf(
			"IMFAttributes::GetGUID failed: 0x%08X", uint32(r1),
		)
	}
	return val, nil
}

// ListCameras enumerates all active video capture devices via Windows Media
// Foundation without opening video streams or illuminating privacy LEDs.
//
// If 0 capture devices are found, or if Media Foundation returns a capture
// error, it returns ErrNoCamerasAttached (error code 0xA00F4244).
func ListCameras() ([]Info, error) {
	...
	defer comRelease(pAttributes)

	if err := comSetGUID(pAttributes, &guidMFDevSourceAttributeSourceType, &guidMFDevSourceAttributeSourceTypeVidCap); err != nil {
	...
	if hr != 0 {
		...
		if uhr == ErrorCodeNoCamerasAttached ||
			hr == HResultNoCamerasAttached ||
			uhr == 0xC00DABE0 ||
			uhr == 0x80070490 ||
			uhr == 0xC00D36D5 {
	...
	for i := uint32(0); i < count; i++ {
		...
		friendlyName, _ := comGetAllocatedString(pActivate, &guidMFDevSourceAttributeFriendlyName)
		symLink, _ := comGetAllocatedString(pActivate, &guidMFDevSourceAttributeVidCapSymbolicLink)
		catGUID, _ := comGetGUID(pActivate, &guidMFDevSourceAttributeVidCapCategory)
		hwSource, _ := comGetUINT32(pActivate, &guidMFDevSourceAttributeVidCapHWSource)

		// Safely release IMFActivate without calling ActivateObject (prevents LED flash).
		...
		deck.Infof(
			"Found Media Foundation camera [%d/%d]: Name=%q, SymbolicLink=%q, IsMIPI=%v, Hardware=%v, Category=%s",
...
// ListActiveDeviceInterfaces enumerates currently active PnP device interfaces
// registered under ksCategoryVideoCamera, ksCategoryCapture,
// guidDevInterfaceCamera, and guidDevInterfaceImage.
func ListActiveDeviceInterfaces() ([]string, error) {
	...
	deck.Infof("Querying PnP device interfaces...")
...
// ListPnPDevices queries the Windows PnP tree for Camera, Image, System, and
// Sensor devices, retrieving status, problem codes, driver INF names, and IDs.
func ListPnPDevices() ([]PnPDevice, error) {
	...
	deck.Infof("Scanning PnP device tree for Camera/Image/System/Sensor classes...")
	...
	for _, classGUID := range classes {
		...
		for i := 0; ; i++ {
			...
			if !isRelevantPnPDevice(
				friendlyName, deviceDesc, instanceID, infPath, classGUID,
			) {
				continue
			}
			seen[key] = true
			...
			deck.Infof(
				"Found PnP Device [%s]: %s (InstanceID: %s, ProblemCode: %d - %s)",
...
// GetDeviceFriendlyName retrieves the friendly name or device description of a
// PnP device node from device properties or device registry properties.
func GetDeviceFriendlyName(devInfo windows.DevInfo, devInfoData *windows.DevInfoData) string {
	val, err := setupDiGetDeviceProperty(devInfo, devInfoData, &devpkeyDeviceFriendlyName)
	if err != nil {
		deck.Warningf("Failed to get device friendly name property: %v", err)
	} else if s, ok := val.(string); ok && strings.TrimSpace(s) != "" {
		return strings.TrimSpace(s)
	}
	val, err = setupDiGetDeviceRegistryProperty(devInfo, devInfoData, windows.SPDRP_FRIENDLYNAME)
	if err != nil {
		deck.Warningf("Failed to get device friendly name registry property: %v", err)
	} else if s, ok := val.(string); ok && strings.TrimSpace(s) != "" {
		return strings.TrimSpace(s)
	}
	val, err = setupDiGetDeviceProperty(devInfo, devInfoData, &devpkeyDeviceDeviceDesc)
	if err != nil {
		deck.Warningf("Failed to get device description property: %v", err)
	} else if s, ok := val.(string); ok && strings.TrimSpace(s) != "" {
		return strings.TrimSpace(s)
	}
	val, err = setupDiGetDeviceRegistryProperty(devInfo, devInfoData, windows.SPDRP_DEVICEDESC)
	if err != nil {
		deck.Warningf("Failed to get device description registry property: %v", err)
	} else if s, ok := val.(string); ok && strings.TrimSpace(s) != "" {
		return strings.TrimSpace(s)
	}
	return ""
...
// GetDeviceDriverInfPath retrieves the driver INF filename for a PnP device node.
func GetDeviceDriverInfPath(devInfo windows.DevInfo, devInfoData *windows.DevInfoData) (string, error) {
	...
	if s, ok := val.(string); ok {
		return filepath.Base(strings.TrimSpace(s)), nil
	}
	return "", nil
}

// GetDeviceDriverVersion retrieves the driver version string for a PnP device.
...
func getDeviceNames(devInfo windows.DevInfo, devInfoData *windows.DevInfoData) (friendlyName, deviceDesc string) {
	friendlyName = GetDeviceFriendlyName(devInfo, devInfoData)
	val, err := setupDiGetDeviceProperty(devInfo, devInfoData, &devpkeyDeviceDeviceDesc)
	if err != nil {
		deck.Warningf("Failed to get device description property: %v", err)
	} else if s, ok := val.(string); ok && strings.TrimSpace(s) != "" {
		deviceDesc = strings.TrimSpace(s)
	}
	if deviceDesc == "" {
		val, err := setupDiGetDeviceRegistryProperty(devInfo, devInfoData, windows.SPDRP_DEVICEDESC)
		if err != nil {
...
// Diagnose executes a comprehensive health check across Media Foundation,
// PnP device nodes, and device interfaces, returning a detailed diagnostic
// report with root-cause analysis and remediation steps for error 0xA00F4244.
func Diagnose() (*DiagnosticReport, error) {
	...
	if report.HasActiveCameras {
		...
		report.Summary = fmt.Sprintf("Camera subsystem is healthy. Found %d active video capture device(s): %s.", len(cameras), strings.Join(names, ", "))
	...
	if len(mipiStatus.DiscoveredComponents) > 0 {
		if !mipiStatus.AllComponentsHealthy {
			report.Summary = fmt.Sprintf("Error 0xA00F4244 <NoCamerasAreAttached>: Internal MIPI camera components have device errors: %s.", strings.Join(mipiStatus.MissingOrFaulty, "; "))
