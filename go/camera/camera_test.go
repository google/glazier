...
func TestProblemCodeDescription(t *testing.T) {
	tests := []struct {
		...
		{code: 0, want: ProblemDescOK},
		{code: 1, want: ProblemDescNotConfigured},
		{code: 10, want: ProblemDescFailedStart},
		{code: 14, want: ProblemDescNeedRestart},
		{code: 18, want: ProblemDescReinstall},
		{code: 22, want: ProblemDescDisabled},
		{code: 24, want: ProblemDescDeviceNotThere},
		{code: 28, want: ProblemDescFailedInstall},
		{code: 31, want: ProblemDescFailedPostStart},
		{code: 39, want: ProblemDescDriverFailedLoad},
		{code: 43, want: ProblemDescCode43Stopped},
		{code: 48, want: ProblemDescDriverBlocked},
...
func TestIsMIPICamera(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			identifiers: []string{
				"Intel(R) Camera Sensor Discrete",
				`ACPI\INT3472\0`,
				"iactrllogic64.inf",
			},
			want: true,
		...
		devices := []PnPDevice{
			{
				...
				Class:              "Camera",
				ProblemCode:        0,
				ProblemDescription: ProblemDescOK,
			},
			{
		...
		devices := []PnPDevice{
			{
				...
				Class:              "Camera",
				ProblemCode:        10,
				ProblemDescription: ProblemDescFailedStart,
			},
			{
				FriendlyName:       "Camera Sensor OV08X40",
				DeviceDesc:         "Camera Sensor OV08X40",
