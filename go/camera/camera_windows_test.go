...
func TestListCameras_Success(t *testing.T) {
	...
	// Set up mock vtable for IMFAttributes / IMFActivate
	var attrVtbl [50]uintptr
	attrVtbl[2] = syscall.NewCallback(func(this uintptr) uintptr { return 1 }) // Release
	attrVtbl[24] = syscall.NewCallback(func(this, pKey, pVal uintptr) uintptr { return 0 }) // SetGUID
	...
	friendlyNameStr, _ := windows.UTF16PtrFromString("Intel(R) MTL AVStream Camera")
	symLinkStr, _ := windows.UTF16PtrFromString(`\\?\display#intc1070#...`)

	var activateVtbl [50]uintptr
	activateVtbl[2] = syscall.NewCallback(func(this uintptr) uintptr { return 1 }) // Release
	activateVtbl[7] = syscall.NewCallback(func(this, pKey, pVal uintptr) uintptr { // GetUINT32
		*(*uint32)(unsafe.Pointer(pVal)) = 1
		return 0
	})
	activateVtbl[10] = syscall.NewCallback(func(this, pKey, pVal uintptr) uintptr { // GetGUID
		*(*windows.GUID)(unsafe.Pointer(pVal)) = ksCategoryVideoCamera
		return 0
	})
	activateVtbl[13] = syscall.NewCallback(func(this, pKey, ppStr, pCch uintptr) uintptr { // GetAllocatedString
		key := *(*windows.GUID)(unsafe.Pointer(pKey))
		if key == guidMFDevSourceAttributeFriendlyName {
			*(*uintptr)(unsafe.Pointer(ppStr)) = uintptr(unsafe.Pointer(friendlyNameStr))
			return 0
		}
		if key == guidMFDevSourceAttributeVidCapSymbolicLink {
			*(*uintptr)(unsafe.Pointer(ppStr)) = uintptr(unsafe.Pointer(symLinkStr))
			return 0
		}
		return uintptr(windows.E_NOTIMPL)
	})

	mockActivateCOM := &comObject{vtbl: &activateVtbl}
...
func TestDiagnose_HealthyCamera(t *testing.T) {
	...
	friendlyNameStr, _ := windows.UTF16PtrFromString("Integrated Camera")
	symLinkStr, _ := windows.UTF16PtrFromString(`\\?\usb#vid_04f2&pid_b6d9#...`)
	...
	var activateVtbl [50]uintptr
	activateVtbl[2] = syscall.NewCallback(func(this uintptr) uintptr { return 1 })
	activateVtbl[7] = syscall.NewCallback(func(this, pKey, pVal uintptr) uintptr {
		*(*uint32)(unsafe.Pointer(pVal)) = 1
		return 0
	})
	activateVtbl[10] = syscall.NewCallback(func(this, pKey, pVal uintptr) uintptr {
		*(*windows.GUID)(unsafe.Pointer(pVal)) = ksCategoryVideoCamera
		return 0
	})
	activateVtbl[13] = syscall.NewCallback(func(this, pKey, ppStr, pCch uintptr) uintptr {
		key := *(*windows.GUID)(unsafe.Pointer(pKey))
		if key == guidMFDevSourceAttributeFriendlyName {
			*(*uintptr)(unsafe.Pointer(ppStr)) = uintptr(unsafe.Pointer(friendlyNameStr))
			return 0
...
func TestDiagnose_PnPStatusOk_ZeroCaptureInterfaces(t *testing.T) {
	...
	setupDiEnumDeviceInfo = func(deviceInfoSet windows.DevInfo, memberIndex int) (*windows.DevInfoData, error) {
		if memberIndex < len(mockPnPList) {
			return &windows.DevInfoData{DevInst: windows.DEVINST(memberIndex + 1)}, nil
		}
		return nil, windows.ERROR_NO_MORE_ITEMS
	}

	setupDiGetDeviceProperty = func(deviceInfoSet windows.DevInfo, deviceInfoData *windows.DevInfoData, propertyKey *windows.DEVPROPKEY) (interface{}, error) {
	...
	setupDiGetDeviceRegistryProperty = func(deviceInfoSet windows.DevInfo, deviceInfoData *windows.DevInfoData, property windows.SPDRP) (interface{}, error) {
		idx := int(deviceInfoData.DevInst) - 1
		if idx >= 0 && idx < len(mockPnPList) {
			return mockPnPList[idx].name, nil
		}
	...
	if !strings.Contains(report.Summary, "0xA00F4244") || !strings.Contains(report.Summary, "no active video capture interfaces") {
...
func TestDiagnose_MIPIComponentError(t *testing.T) {
	...
	mockPnPList := []struct {
		...
		{name: "Intel(R) MTL AVStream Camera", instanceID: `ACPI\INTC1070\1`, problemCode: 10},
	}
